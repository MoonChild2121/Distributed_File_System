...
def handle_write(filename, client_id, file_version_map):
    ...
    reply_DS = send_file_manager(client_socket, filename, 'w', False)  # request the file info from directory service
...
def handle_read(filename, file_version_map, client_id):
    client_socket = create_socket()  # create socket to directory service
    reply_DS = send_file_manager(client_socket, filename, 'r', False)  # send file name to directory service
...
def send_file_manager(client_socket, filename, RW, list_files):
    serverName = 'localhost'
    ...
    if not list_files:
        msg = filename + '|' + RW
        # send the string requesting file info to directory send_file_manager
        client_socket.send(msg.encode())
        reply = client_socket.recv(1024)
        reply = reply.decode()
    else:
...
def instructions():
    ...
    print (" write to file with <write> filename")
    print (" to finish writing out in <end>")
    print (" read from file with <read> filename")
    print (" Write <instructions> to see instructions again")
    print (" <quit> to quit")
